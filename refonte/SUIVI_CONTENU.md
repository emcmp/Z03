...
| 2 | Variables et affectation | Variables, affectation, nombres, opérateurs, chaînes, concaténation | Labo 2 → `420905_lab2.docx` | **Théorie 905 conservée en R6; labo réorganisé** | `web/docs/01-cours/06-rencontre6.md` est revenu à la version historique migrée du 905. La navigation native « Sur cette page » repose sur ses titres `##` / `###`. Le labo étudiant réorganisé reste dans `web/docs/02-labos/01-labo2.md` avec 15 activités principales + pratiques facultatives. DOCX, transcription Markdown et captures historiques conservés. Voir `REVISION_R6.md`. |
...
| Variables / chaînes | variables, déclaration, affectation, réaffectation, nombres, chaînes, opérateurs simples, template strings | `+=` et `-=` à pratiquer sans multiplier les variantes | Les déclarations multiples et `++` / `--` restent visibles dans la théorie historique 905, mais ne sont pas prioritaires pour le quiz; détails de priorité au-delà du nécessaire |
...
- R6 conserve maintenant la théorie historique migrée du 905; seul le laboratoire étudiant demeure réorganisé. Voir `REVISION_R6.md`.
